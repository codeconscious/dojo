module Quadratic
    ...
    let quad a b c =
        let discrimant =
            if a = 0.0 then None
            else Some((pown b 2) - (4.0 * a * c)); // b^2 - 4ac
        let rootPos discrimant a b = (-b + sqrt discrimant) / (2.0 * a); // quad eq plus sqrt
        let rootNeg discrimant a b = (-b - sqrt discrimant) / (2.0 * a); // quad eq minus sqrt
